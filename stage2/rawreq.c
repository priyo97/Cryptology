...
{    
    ...
    while(1)
	{
	    ...
	    if( strcmp(inet_ntoa(source.sin_addr),"192.168.0.111") == 0 && tcp -> fin )
            break;
	    ...
	    if( strcmp(inet_ntoa(dest.sin_addr),"192.168.0.111") == 0 )
	    {
    ...
    extract_form_fields(header_portion, header_len, body_portion, &data);
    ...
    for(i = 0 ; i < data.num_form_fields ; i++)
    {
       // printf("Name    : %s\n",data.f[i].name);
        ...
        if(data.f[i].is_file)
        {
            FILE *fptr = fopen(data.f[i].filename,"w");
            
            if( fptr != NULL )
            {
                fwrite(data.f[i].value, sizeof(unsigned char), strlen(data.f[i].value),fptr);
