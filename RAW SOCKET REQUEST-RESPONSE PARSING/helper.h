...
void extract_form_fields(char* header_portion,int header_len,char *body_portion,int body_len, struct form_data *f)
{
    unsigned char *tmp;
    ...
    while(1)
    {
        token = (unsigned char*)memmem( tmp, body_len, delimiter, strlen(delimiter) );
        ...
        if( strcmp(tmp,"") != 0 )
        {
            ...
            if(s != NULL)
            {
                ...
                form_data[i].value = t;
                form_data[i].length = token - t - strlen("\r\n");   
            }
            else
            {
                r = strstr(tmp,"name=\"");
        ...
        body_len -= strlen(delimiter) + strlen("\r\n");
    }
